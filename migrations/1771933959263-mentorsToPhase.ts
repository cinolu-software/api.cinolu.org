...
export class MentorsToPhase1771933959263 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`phase_mentors_mentor_profile\` (\`phaseId\` uuid NOT NULL, \`mentorProfileId\` uuid NOT NULL, INDEX \`IDX_c879efe5441346e6fe3f6011ec\` (\`phaseId\`), INDEX \`IDX_1fc87ce223cc261e7779a3a9ad\` (\`mentorProfileId\`), PRIMARY KEY (\`phaseId\`, \`mentorProfileId\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`phase\` DROP COLUMN \`slug\``);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`phase\` CHANGE \`description\` \`description\` text CHARACTER SET "latin1" COLLATE "latin1_swedish_ci" NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`phase\` ADD \`slug\` varchar(255) CHARACTER SET "latin1" COLLATE "latin1_swedish_ci" NOT NULL`);
