...
export class AddVentureProduct1759220586776 implements MigrationInterface {
    name = 'AddVentureProduct1759220586776'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`product\` (\`id\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`name\` varchar(255) NOT NULL, \`description\` text NOT NULL, \`price\` decimal(10,2) NOT NULL, \`ventureId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`gallery\` ADD \`productId\` varchar(36) NULL`);
        await queryRunner.query(`ALTER TABLE \`gallery\` ADD \`ventureId\` varchar(36) NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`gallery\` DROP COLUMN \`ventureId\``);
        await queryRunner.query(`ALTER TABLE \`gallery\` DROP COLUMN \`productId\``);
